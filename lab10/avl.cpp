//! @file avl.cpp
//! Parallel Lab version of avl.cc g
 ...
 * 
 * @pre word has been initialized to an appropriate default value,  
 *      meaning some valid KType value which is LESS THAN any stored in the tree
 * <UL><LI> for KType string, and assuming empty strings are not in the tree, 
...
void findMaxHelper(Node * root, KType & word, VType & count) {
  ...
  // FORK = CREATE NEW THREAD(S) TO DO TASK
  // JOIN = WAIT FOR ALL THREAD(S) TO FINISH
  // REMEMBER THAT CURRENT THREAD CAN ALSO DO WORK
  if (root == NULL) {
    return;
  }

  const int SEQUENTIAL_CUTOFF = 10; // 2^10 = 1024
...
/** 
 * Runs REPS trials of different types of searching the tree. 
 * 
 ...
 * @param k = a reference to a valid KType variable (which has been appropriately initialized)  
...
int main( int argc, char *argv[] ) {
  ...
  // if none exists, then make one, with value == 1
  // else increment the tally for the word by 1  
